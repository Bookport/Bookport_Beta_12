...
const SYSTEM_KEYS: SystemKey[] = [
  ...
  },
  {
    ...
    superlevelText: "400-600 г (5 кнопок) — при высокой активности.",
  ...
  },
  {
    ...
    optimumText: "1 «комплимент» в день (одно блюдо на выбор из категории блюд «Комплименты недели» (из любой недели курса)).",
  ...
  },
  {
    ...
    portionSize: "ОБЯЗАТЕЛЬНАЯ ЗАДАЧА: при использовании бобовых, цельных злаков, семян и орехов.",
    ...
    specialNote: "ПРАВИЛА: Бобовые: 8–12ч в холодной воде с яблочным уксусом (1 ст.л. на 1л). Перед варкой промыть. Злаки: 5–8ч (с лимонным соком). Орехи: 4–8ч в чистой воде (без кислоты). Семена льна/чиа: для получения геля. Масличные семена: 4–6ч с лимоном.",
  },
  {
    id: "no_oil_cook",
    num: 17,
    ...
    portionSize: "ДНЕВНАЯ ЗАДАЧА: все горячие блюда приготовлены абсолютно без добавления масла.",
    optimumText: "все горячие блюда приготовлены абсолютно без добавления масла.",
    whyImportant: "масла — это рафинированные, изолированные жиры, лишённые клетчатки, фитонутриентов и большинства витаминов. Они вызывают постпрандиальное воспаление; повышают нагрузку на печень; нарушают чувствительность вкусовых рецепторов; при нагреве окисляются. В цельном растительном рационе все необходимые жиры организм получает из цельных продуктов: орехов, семян, авокадо, оливок."
  },
  {
    id: "no_salt_cook",
    num: 18,
    ...
    whatsIncluded: "приготовление любых блюд без добавления поваренной соли, морской, гималайской или любой другой соли. Соль не используется ни в процессе готовки, ни при подаче. Вкус блюд формируется за счёт специй, трав, кислоты, овощного бульона, водорослей и ферментированных продуктов.",
    portionSize: "ДНЕВНАЯ ЗАДАЧА: все блюда приготовлены полностью без добавления соли.",
    optimumText: "все блюда приготовлены полностью без добавления соли.",
    whyImportant: "соль (NaCl) — при введении изолированного NaCl повышается артериальное давление и нагрузка на почки; возникает задержка жидкости и отёки; нарушается чувствительность вкусовых рецепторов; соль стимулирует аппетит, вызывая переедание. Через 7–10 дней без соли вкусовые сосочки обновляются, и вы начинаете различать естественную сладость и глубину вкуса продуктов без «усилителя»."
  },
  {
    id: "no_caffeine_day",
    num: 19,
    ...
    whatsIncluded: "полное исключение из рациона всех напитков и продуктов, содержащих кофеин. Под запрет попадают: кофе (включая декофеинизированный), чёрный и зелёный чай, мате, матча, какао в больших количествах, энергетические напитки, а также продукты с добавлением кофеина.",
    portionSize: "ДНЕВНАЯ ЗАДАЧА: полное отсутствие источников кофеина на протяжении дня.",
    optimumText: "полное отсутствие источников кофеина на протяжении дня.",
    whyImportant: "кофеин — это психоактивный стимулятор, который повышает уровень кортизола; нарушает циркадные ритмы и качество сна; создаёт зависимость; раздражает слизистую ЖКТ; может усиливать тревожность.",
    specialNote: "ПОМОЩЬ СЕБЕ: чем заменить кофеин: травяные чаи (ромашка, мята, мелисса, липа, шиповник, иван-чай); цикорий; тёплая вода с лимоном; настои из сушёных ягод и фруктов. В первые дни отказа возможна головная боль — это нормальная реакция отмены.",
  },
  {
    id: "no_sugar_day",
    num: 20,
    ...
    portionSize: "ДНЕВНАЯ ЗАДАЧА: полный отказ от чистого или добавленного сахара, а также любых сахарозаменителей.",
