...
export const ALIASES: Record<string, string> = {
  ...
  // --- ЗЕЛЁНЫЕ СИРОТЫ, ПРИВЯЗАННЫЕ К БД (нестыковки в названиях) ---
  "банан сушен": "банан сушеный",
  "имбирь молотый": "имбирь (молотый)",
  "имбирь корень,": "имбирь корень",
  "инжир (фига) сушеный, вяленый": "инжир сушеный",
  "инжир (фига) сушеный, сырой": "инжир сушеный",
  "капуста краснокачанная": "капуста краснокочанная",
  "капуста пекинская": "пекинская капуста",
  "салат-латук": "салат латук",
  "тыквы и кабачка семя": "семена тыквы",
  "рис черный": "черный рис",
  "яблоко сушен": "яблоки сушеные",
  "сушеные яблоки": "яблоки сушеные",
  "мята курчавая сушеная": "мята сушеная",

  // Точные алиасы составных блюд (иначе обобщённые базовые слова
  // ошибочно резолвятся, напр. "гречневая каша" → "гречневая мука").
  "гречневая каша": "гречка",

  // Слэш нельзя использовать в имени файла, поэтому макароны/спагетти
  // маппим на единое имя файла без слэша.
  "пшеничные изделия гот. макароны/спагетти": "пшеничные изделия гот. макароны",
  "баклажан солен./марин": "баклажан солен.марин",
  "свекла солен./марин": "свекла солен.марин",
};
...
export const MODIFIER_WORDS: ReadonlySet<string> = new Set<string>([
...
// Ищет нормализованный ключ изображения по имени, используя те же шаги, что и
// getIngredientImage: точное имя/модификаторы/усечение/базовые слова (+ ALIASES),
// затем fallback в ingredientAliasMapper. Возвращает найденный ключ или null.
export function resolveAgainstIndex(name: string, index: Set<string>): string | null {
  if (!name) return null;
  ...
  for (const candidate of candidateKeys(name)) {
    ...
    if (aliased) {
      ...
      if (index.has(aliasKey)) return aliasKey;
    }
  }
  ...
  if (resolved !== name) {
    ...
    if (index.has(resolvedKey)) return resolvedKey;
    const aliased2 = ALIASES[resolved];
    if (aliased2) {
      const aliasKey2 = normalize(aliased2);
      if (index.has(aliasKey2)) return aliasKey2;
    }
  }
