...
export const MOVEMENT_PHRASES = {
  ...
  // 4.4 Малоподвижность + Низкий тонус -> тело в экономии, начни с малого, чтобы не уйти в ноль
  movementSedentaryTonusLow: [
    (ctx: MovementContext) => `Вижу, что тонус сегодня тяжелый и энергии мало, но ${ctx.activeMinutes} минут движения недостаточно. Энергия рождается в действии — сделай хотя бы легкую разминку, чтобы взбодриться!`,
    (ctx: MovementContext) => `Вижу по общей сводке: движения почти нет (${ctx.activeMinutes} мин) и тонус по замерам снижен. Тело в режиме экономии — начни с 10-минутной разминки, это разбудит и мышцы, и энергию.`,
    (ctx: MovementContext) => `Сидячий день (${ctx.activeMinutes} мин) плюс низкий тонус аукаются: застой понижает и силы, и обмен веществ. Лёгкая прогулка сейчас даст больше, чем кажется!`,
  ],
