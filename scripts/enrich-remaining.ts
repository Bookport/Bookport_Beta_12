...
async function main() {
  // Items where tryptophan is still 0 (not enriched yet)
  const items = await prisma.foodItem.findMany({
    where: { tryptophan: 0, fdcId: { not: null } },
    orderBy: { nameRu: "asc" },
  });

  console.log(`Found ${items.length} remaining items. Starting...`);
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    try {
      ...
      failed++;
      console.warn(`  [${i + 1}/${items.length}] ${item.nameRu} x ${(err as any)?.message || err}`);
    }
